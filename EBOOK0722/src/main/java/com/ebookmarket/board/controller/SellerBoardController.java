...
import com.ebookmarket.board.domain.Criteria;

import com.ebookmarket.board.domain.PageMaker;
import com.ebookmarket.board.domain.SearchCriteria;
...
public class SellerBoardController {
	...
	@PreAuthorize("hasRole('ROLE_seller')")
	public void sellerBoardRegister(Model model, Authentication authentication) throws Exception {
		// 로그인한 사용자 정보 획득
...
//회원 일련번호가맞지않으면 접속이 불가능
		if (s_num != sellerBoard.getS_num()) {
			return "sellerBoard/ERROR";
		} else {
			model.addAttribute(sellerBoard);
