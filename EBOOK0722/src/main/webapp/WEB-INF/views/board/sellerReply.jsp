...
<script type="text/javascript">
	...
	function isNumber(input) {
		if ($(input).val() != "") {
			var nus_regExp = new RegExp("[0-9]$", "i"); //only number
			var match = nus_regExp.exec($(input).val());
			//alert(match);
			if (match == null) {
				alert("숫자를 입력해주세요");
				$(input).val(
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#replyInsert").click(
				function() {
					...
					if (!chkData("#sr_title", "이름을"))
						return;

					else if (!chkData("#sr_pw", "비밀번호를"))
						return;

					else if (!chkData("#sr_content", "내용을"))
						return;

					else {
						var insertUrl = "/sellerReplies/replyInsert";
		...
		$(document).on(
				...
				".update_btn",
				function() {
					var sr_num = $(this).parents("li").attr("data-num");
					var sr_content = $("#content").val();
					if (!chkData("#content", "댓글 내용을"))
						return;
					else {
						$.ajax({
							url : '/sellerReplies/' + sr_num + ".do", //전송 url
							beforeSend : function(xhr) {
								xhr.setRequestHeader("${_csrf.headerName}",
										"${_csrf.token}");
							},
					...
					if (pwdConfirm == 0) {
						...
						if (confirm("선택하신 댓글을 삭제하시겠습니다까?")) {
							$.ajax({
								...
								url : "/sellerReplies/" + replyNum + ".do", //전송 url
								beforeSend : function(xhr) {
									xhr.setRequestHeader("${_csrf.headerName}",
											"${_csrf.token}");
								},
		...
		$(document).on(
				...
				function() {
					...
					if (!formCheck(pwd, r_msg, "비밀번호를"))
						...
						$
								.ajax({
									url : "/sellerReplies/pwdConfirm.do", //전송 url
									beforeSend : function(xhr) {
	...
	//리스트 요청 함수
	function listAll(sb_num) {

		$("#comment_list").html("");
		var url = "/sellerReplies/all/" + sb_num + ".do";
		$.getJSON(url, function(data) {
			console.log(data.length);

			$(data).each(function() {
				var sr_num = this.sr_num;
				var sr_title = this.sr_title;
				var sr_content = this.sr_content;
				var sr_reg_date = this.sr_reg_date;
				addNewItem(sr_num, sr_title, sr_content, sr_reg_date);
			});
		}).fail(function() {
	...
	function addNewItem(sr_num, sr_title, sr_content, sr_reg_date) {
		//새로운 글이 추가될 li태그 객체
		var new_li = $("<li>");
		new_li.attr("data-num", sr_num);
		...
		name_span.html(sr_title + "님");

		//작성 일시
		var date_span = $("<span>");
		date_span.html(" / " + sr_reg_date + " ");
	...
	<div id="replyContainer">
		...
		<div id="comment_write">
			<form id="comment_form">
				<div>
					<label for="sr_title">작성자</label> <input type="text"
						name="sr_title" id="sr_title" /> <label for="sr_pw">비밀번호</label>
					<input type="password" name="sr_pw" id="sr_pw" /> <input
						type="button" id="replyInsert" value="저장하기" />
				</div>
				<div>
					<label for="sr_contnet">덧글내용</label>
					<textarea name="sr_content" id="sr_content"></textarea>
				</div>
			</form>
		</div>
		<ul id="comment_list">
